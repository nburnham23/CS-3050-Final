...
class Game:
    ...
    def is_checkmate(self, color):
        ...
        king_pos = self.find_king(color)
        # Loop through every piece that is your own
        for r in range(8):
            for c in range(8):
                piece = self.board.get_piece((r, c))
                # If the piece is your piece
                if piece and piece.piece_color == self.current_turn:
                    ...
                    for move in piece.moveset:
                        ...
                        original_position = (r, c)
                        # Make the move temporarily
                        target_position = move # TODO: Instead of every move, move piece to a targeted position
                        captured_piece = self.board.get_piece(target_position)
                        self.board.move(original_position, target_position)
                        # Check if still in check
                        in_check = self.is_in_check(color)
                        # Undo the move
                        self.board.move(target_position, original_position)
                        if captured_piece:
                            self.board.set_piece(target_position, captured_piece)
                        # If not in check after the move then not checkmate
                        if not in_check:
                            return False
        return True
